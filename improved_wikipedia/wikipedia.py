...
def pages(titles, pageid=None, auto_suggest=True, redirect=True, preload=False):
    ...
    request = _wiki_request(query_params)
    for pid in request["query"]["pages"]:
        if int(pid) == -1:
            continue
        d = deepcopy(request)

        d["query"]["pages"] = {pid: request["query"]["pages"][pid]}
        try:
            p = WikipediaPage(content=d)
            pages.append(p)
...
class WikipediaPage(object):
    ...
    def __init__(self, title=None, pageid=None, redirect=True, preload=False, original_title='', content=None):
        if title is not None:
            self.title = title
        ...
        self.__load(redirect=redirect, preload=preload, content=content)
    ...
    def __load(self, redirect=True, preload=False, content=None):
        ...
        elif 'redirects' in query and content is None:
            if redirect:
